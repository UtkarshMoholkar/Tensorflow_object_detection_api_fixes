...
def train_loop(
    ...
    # higher variable scope to enter inner scopes
    control = False
    # largest weight to progressively lower
        ...
        with strategy.scope():
            with tf.compat.v2.summary.record_if(
                ...
                for _ in range(global_step.value(), train_steps,
                    ...
                    # at every hundred steps, save evaluation metrics
                    if global_step.value() - logged_step >= 100:
                        ...
                        if loss_val < 0.95 and loss_val>0.15:
                            tf.logging.info(
                                'Step {} per-step time {:.3f}s'.format(
                                    global_step.value(), time_taken / num_steps_per_iteration))
                            tf.logging.info(pprint.pformat(
                                logged_dict_np, width=40))
                            logged_step = global_step.value()
                            print("model stops on best loss_val of 0.2")
                            checkpointed_step = int(global_step.value())
                            manager.save()
                            control = True
                            break
